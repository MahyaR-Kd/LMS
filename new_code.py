...
def verify_code_generator(email):
    ...
    if email_exist:
        ...
        cur.execute("UPDATE users SET token=%s WHERE ID=%s;" % (verify_code,usr_id,))
        print("UPDATE users SET token=%s WHERE ID=%s;" % (verify_code,usr_id,))
        cur.execute("select firstname from users where to_tsvector(email) @@ to_tsquery(%s);", (email,))
        usr_firstname = cur.fetchall()
        usr_firstname = usr_firstname[0][0]
        usr_firstname = usr_firstname.replace(' ','')
        cur.execute("select lastname from users where to_tsvector(email) @@ to_tsquery(%s);", (email,))
        ...
        flash('Email Sended Please Check Your Email Inbox')
